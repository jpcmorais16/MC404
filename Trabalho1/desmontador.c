...
void RemoveZerosDaFrente(unsigned char *string)
{
	...
	for (int i = 0; i < tamanho; i++)
	{
		...
		if (gatilho == 1)
			ResultadoRemocao[i - defasagem] = string[i];
	}
...
void HexBin(unsigned char *hexa)
{
	...
	for (int i = 0; i < 32; i += 4)
	{
		...
		case 'f':
			AdicionaAoBinario("1111");
			break;
		case 'e':
			AdicionaAoBinario("1110");
			break;
		case 'd':
			AdicionaAoBinario("1101");
			break;
		case 'c':
			AdicionaAoBinario("1100");
			break;
		case 'b':
			AdicionaAoBinario("1011");
			break;
		case 'a':
			AdicionaAoBinario("1010");
			break;
		case '9':
			AdicionaAoBinario("1001");
			break;
		case '8':
			AdicionaAoBinario("1000");
			break;
		case '7':
			AdicionaAoBinario("0111");
			break;
		case '6':
			AdicionaAoBinario("0110");
			break;
		case '5':
			AdicionaAoBinario("0101");
			break;
		case '4':
			AdicionaAoBinario("0100");
			break;
		case '3':
			AdicionaAoBinario("0011");
			break;
		case '2':
			AdicionaAoBinario("0010");
			break;
		case '1':
			AdicionaAoBinario("0001");
			break;
		case '0':
			AdicionaAoBinario("0000");
			break;
		default:
			break;
		}
...
void DescobreInstrucao(unsigned char *elf, unsigned int offset)
{
	...
	if (SizedStrComp("1110110", Binario, 0, 7) == 0)
		CopiaInstrucao("lui");
	else if (SizedStrComp("1110100", Binario, 0, 7) == 0)
		CopiaInstrucao("auipc");
	else if (SizedStrComp("1111011", Binario, 0, 7) == 0)
		CopiaInstrucao("jal");
	else if (SizedStrComp("1110011", Binario, 0, 7) == 0)
		CopiaInstrucao("jalr");
	...
	else if (SizedStrComp("1100011", Binario, 0, 7) == 0)
	{
		...
		if (SizedStrComp("000", Binario, 12, 3) == 0)
			CopiaInstrucao("beq");
		else if (SizedStrComp("100", Binario, 12, 3) == 0)
			CopiaInstrucao("bne");
		else if (SizedStrComp("001", Binario, 12, 3) == 0)
			CopiaInstrucao("blt");
		else if (SizedStrComp("101", Binario, 12, 3) == 0)
			CopiaInstrucao("bge");
		else if (SizedStrComp("011", Binario, 12, 3) == 0)
			CopiaInstrucao("bltu");
		else if (SizedStrComp("111", Binario, 12, 3) == 0)
			CopiaInstrucao("bgeu");
		else
	...
	else if (SizedStrComp("1100000", Binario, 0, 7) == 0)
	{
		...
		if (SizedStrComp("000", Binario, 12, 3) == 0)
			...
			CopiaInstrucao("lh");
		else if (SizedStrComp("010", Binario, 12, 3) == 0)
			CopiaInstrucao("lw");
		else if (SizedStrComp("001", Binario, 12, 3) == 0)
			CopiaInstrucao("lbu");
		else if (SizedStrComp("101", Binario, 12, 3) == 0)
	...
	else if (SizedStrComp("1100100", Binario, 0, 7) == 0)
	{
		...
		if (SizedStrComp("000", Binario, 12, 3) == 0)
			...
			CopiaInstrucao("slti");
		else if (SizedStrComp("110", Binario, 12, 3) == 0)
			CopiaInstrucao("sltiu");
		else if (SizedStrComp("001", Binario, 12, 3) == 0)
			CopiaInstrucao("xori");
		else if (SizedStrComp("011", Binario, 12, 3) == 0)
			CopiaInstrucao("ori");
		...
		else if (SizedStrComp("101", Binario, 12, 3) == 0)
		{
			...
			if (SizedStrComp("0000000", Binario, 25, 7) == 0)
				CopiaInstrucao("srli");
			else if (SizedStrComp("0000010", Binario, 25, 7) == 0)
				CopiaInstrucao("srai");
			else
				CopiaInstrucao("<unknown>");
		}
	...
	else if (SizedStrComp("1111000", Binario, 0, 7) == 0)
	{
		...
		if (SizedStrComp("000", Binario, 12, 3) == 0)
			CopiaInstrucao("fence");
		else if (SizedStrComp("100", Binario, 12, 3) == 0)
			CopiaInstrucao("fence.i");
		else
			CopiaInstrucao("<unknown>");
	}
	...
	else if (SizedStrComp("1100111", Binario, 0, 7) == 0)
	{
		...
		if (SizedStrComp("000", Binario, 12, 3) == 0)
		{
			...
			if (SizedStrComp("0000000", Binario, 25, 7) == 0)
				CopiaInstrucao("ecall");
			else if (SizedStrComp("1000000", Binario, 25, 7) == 0)
				CopiaInstrucao("ebreak");
			else
				CopiaInstrucao("<unknown>");
		}

		if (SizedStrComp("100", Binario, 12, 3) == 0)
			CopiaInstrucao("csrrw");
		else if (SizedStrComp("010", Binario, 12, 3) == 0)
			CopiaInstrucao("csrrs");
		else if (SizedStrComp("110", Binario, 12, 3) == 0)
			CopiaInstrucao("csrrc");
		else if (SizedStrComp("101", Binario, 12, 3) == 0)
			CopiaInstrucao("csrrwi");
		else if (SizedStrComp("011", Binario, 12, 3) == 0)
			CopiaInstrucao("csrrsi");
		else if (SizedStrComp("111", Binario, 12, 3) == 0)
			CopiaInstrucao("csrrci");
		else
			CopiaInstrucao("<unknown>");
	}
	else
...
int main(int argc, char *argv[])
{
	...
	//int fd = open("bin/test-29.x", O_RDONLY);
	Elf32_Ehdr *header;
	...
	//if (0){
		...
		for (int i = 1; i < SymtabHeader->sh_size / 16; i++)
		{
			...
			while (contadorPontos < SymbolsList[i]->st_shndx)
			{
				...
				if (*(elf + HeaderShstrtab->sh_offset + it) == '.')
					contadorPontos += 1;
	...
	//else if (1){
		...
		for (int i = 0; i < SymtabHeader->sh_size; i += 16)
		{
			...
			for(int j=0; j < TamanhoString(elf + SymbolsList[i / 16]->st_name + StrtabHeader->sh_offset); j++){
		...
		for (int i = 1; i < TamanhoLista && offset < TextHeader->sh_size; i++)
		{
			...
			if (StrComp(ResultadoOrdenacao[i].str1, ResultadoConversao) >= 0)
			{
				...
				{
					if (i < TamanhoLista - 1 && linhaText >= HexDec(ResultadoOrdenacao[i + 1].str1))
						break;
